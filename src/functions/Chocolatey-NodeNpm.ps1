...
param(
  ...
  if ($($env:Path).ToLower().Contains("nodejs") -eq $false) {
    $env:Path = Get-EnvironmentVariable -Name 'Path' -Scope Machine
  }


  Write-Host "Chocolatey (v$chocVer) is installing $packageName and dependencies (using npm.org). By installing you accept the license for $packageName and each dependency you are installing." -ForegroundColor $RunNote -BackgroundColor Black


  $chocoInstallLog = Join-Path $nugetChocolateyPath 'chocolateyNodejsInstall.log';
  Append-Log $chocoInstallLog

  $packageArgs = "/c npm install $packageName"
  if ($version -notlike '') {
